...
describe('Produto', () => {
  ...
  describe('Criar', () => {
    it('Descrição Produto é obrigatório', (done) => {
      ...
      Promise.resolve(result).then((res) => {
        ...
        res.body.should.have.property('message').eql('Descrição Produto é obrigatório')
  ...
  describe('Alterar', () => {
    it('Id não foi informado para alteracção', (done) => {
      ...
      var result = chai.request(server).put('/'+baseUrl).send(listaProduto[2])

      Promise.resolve(result).then((res) => {
        res.should.have.status(401)
        res.body.should.have.property('message').eql('Id não foi informado')
        done()
      }).catch(done)
    })
